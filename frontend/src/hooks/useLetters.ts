...
export function useWhyInterestedHighlights(jobId: string | undefined) {
  const { userId } = useAuth();
  return useMutation({
    mutationFn: () =>
      api.post<WhyInterestedHighlightsResponse>(
        `/users/${userId}/jobs/${jobId}/why-interested/highlights`,
      ),
  });
}
