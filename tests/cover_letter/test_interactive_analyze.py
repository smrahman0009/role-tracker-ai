...
def test_analyze_uses_sonnet_by_default() -> None:
    """Restoration default is Sonnet (was Haiku pre-deletion); the
    panel's model toggle lets the user downgrade per call."""
    client = _StubClient(_valid_payload())
    analyze("r", "j", client=client)
    assert client.messages.last_request["model"] == "claude-sonnet-4-6"


def test_analyze_honours_explicit_haiku() -> None:
    client = _StubClient(_valid_payload())
    analyze("r", "j", client=client, model="claude-haiku-4-5")
